...
from mahalo.media_bundler.conf import bundler_settings
from mahalo.media_bundler.bin_packing import Box, pack_boxes
from mahalo.media_bundler.jsmin import jsmin
# from mahalo.media_bundler.cssmin import minify_css
from mahalo.media_bundler.yuicssmin import cssmin
from mahalo.media_bundler import versioning
...
class CssBundle(Bundle):
    ...
    def _make_bundle(self):
        minifier = cssmin if self.minify else None
        self.do_text_bundle(minifier)
    ...
    def do_text_bundle(self, minifier=None):
        with open(self.get_bundle_path(), "w") as output:
            ...
            for path in self.get_paths():
                if '.less' in path[-5:]:
                    compiled_path = '%s%s' % (path[:-5], self.get_extension())
                    os.system('lessc '+ path +' '+ compiled_path)
                    paths.append(compiled_path)
                else: 
                    paths.append(path)

            generator = concatenate_files(paths)
            if minifier:
                # Eventually we should use generators to concatenate and minify
                # things one bit at a time, but for now we use strings.
                output.write(minifier("".join(generator)))
